...
public class carpisma_photon : MonoBehaviourPunCallbacks
{
    ...
    public static int anlama=1,hediyee,ates_boyut=1,ates_boyut2=1,zamanbomb=0,tekme=0,x,y;
    ...
    void Start()
    { 
        GameObject.Find("yonet").GetComponent<PhotonView>().RPC("yikilacakobje",RpcTarget.All,null);
        oyuncu_sp = GetComponent<SpriteRenderer>();
        pw = GetComponent<PhotonView>();
        anlama=1;
        if(PhotonNetwork.IsMasterClient){
            photonView.RPC("ilkboyut",RpcTarget.All);
        }
     ...
     [PunRPC]
    public void yikil(int j){
       yonet.yikilanlar[j].transform.position = new Vector3 (-50,-50,0);
    }
    [PunRPC]
    ...
    void ates_size(string amac){
        ...
        if(amac =="arti"){
            ates_boyut++;
            GameObject.FindWithTag("player2"). GetComponent<PhotonView>().RPC("boyut_degis2",RpcTarget.All,1,"arttır");
            ...
            if(ates_boyut>=2){
                ates_boyut--;
                GameObject.FindWithTag("player2"). GetComponent<PhotonView>().RPC("boyut_degis2",RpcTarget.All,1,"az");
                foreach (Transform child in ates.transform)
                {
                    if(child.tag == "sag"){
                        child.position += new Vector3 (-0.5f,0,0);
    ...
    void hediye_islemler(string islem){
        ...
        if(!photonView.IsMine) return;
        if(islem=="bomba"){
            Bomb_Asil.bombsayi+=1;
        }
        else if(islem=="hiz"){
            hareket_online.hiz+=0.5f;  
        }
        else if(islem=="kick"){  
            tekme=1;  
        }
