...
using Photon.Pun;
public class hareket_online : MonoBehaviourPunCallbacks
{
    ...
    // [SerializeField] private enum SpriteState{
    //     sp_up,sp_down,sp_left,sp_right
    // }
    ...
    void isin(){
        if(Input.GetMouseButtonDown(0)){
            ...
            if(Physics.Raycast(transform.position,transform.forward,out hit, 100f)){
                hit.collider.gameObject.GetComponent<PhotonView>().RPC("derbever",RpcTarget.All,20);
            }
        }
    }
    [PunRPC]
