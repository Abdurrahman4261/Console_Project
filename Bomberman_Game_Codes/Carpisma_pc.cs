...
public class Carpisma_pc : MonoBehaviour
{
    public Camera Anacam;
    public GameObject Ikincicam;
    public Button kayıp;
    ...
    public int sart_death=0;
    public Rigidbody2D coll;
    public GameObject ates,death1,p1;
    private Vector3 boyut,pozisyon;
    public TMP_Text kazanma,kaybetme;
    public GameObject kaybet_buton,kazan_buton;
    ...
    private void OnTriggerEnter2D(Collider2D duvar) {
        ...
        if(duvar.gameObject.tag=="Finish" && enemy.gecis==4){
            choose_music(6);
            Time.timeScale=0;
            kazan_buton.SetActive(true);
            new WaitForSeconds(2f);
            kazanma.text="Sonraki Bölüm";
        }
    ...
    private IEnumerator Ölü(){
        if(sart_death==1){
            ...
            yield return new WaitForSeconds(0.2f);
            kaybetme.text="Yeninden Deneyin";
            kayıp.enabled=true;
            Destroy(player1);
            Time.timeScale=0;
        }
