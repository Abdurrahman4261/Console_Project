...
public class Bomb : MonoBehaviour
{
    ...
    private IEnumerator asagi(GameObject ates,Vector3 bomba,Vector3Int pPos,int sayacc){
        ...
        for(;sayacc<Carpisma.ates_boyut;sayacc++){
            ...
            if(random_sayi==0){
                ...
                shediye_tekme=hediye_al_as(bomba,sayacc,shediye_tekme,hediye_tekme);
            } 
    ...
    private IEnumerator yuk(GameObject ates,Vector3 bomba,Vector3Int pPos,int sayacc){
        ...
        for(;sayacc<Carpisma.ates_boyut;sayacc++){
            if(random_sayi==0){
                ...
                shediye_tekme=hediye_al_yuk(bomba,sayacc,shediye_tekme,hediye_tekme);
            } 
            else if(random_sayi==5){
    ...
    private IEnumerator sol(GameObject ates,Vector3 bomba,Vector3Int pPos,int sayacc){
        ...
        for(;sayacc<Carpisma.ates_boyut;sayacc++){
            if(random_sayi==0){
                ...
                shediye_tekme=hediye_al_sol(bomba,sayacc,shediye_tekme,hediye_tekme);
            } 
            else if(random_sayi==5){
    ...
    private IEnumerator sag(GameObject ates,Vector3 bomba,Vector3Int pPos,int sayacc){
        ...
        for(;sayacc<Carpisma.ates_boyut;sayacc++){
            if(random_sayi==0){
                ...
                shediye_tekme=hediye_al_sag(bomba,sayacc,shediye_tekme,hediye_tekme);
            } 
            else if(random_sayi==5){
    ...
    private IEnumerator PlaceBomb(Vector3 bombapos){
        ...
        uzaklıküst(Carpisma.ates_boyut) ;uzaklıksag(Carpisma.ates_boyut);    ;uzaklıksag2(Carpisma.ates_boyut); uzaklıküst2(Carpisma.ates_boyut); 
        uzaklıkalt(Carpisma.ates_boyut) ;uzaklıksol(Carpisma.ates_boyut);    uzaklıkalt2(Carpisma.ates_boyut) ;uzaklıksol2(Carpisma.ates_boyut);
        
        GameObject ates = Instantiate(atesprefab,bombapos,Quaternion.identity);
        
        for(sayac=0;sayac<Carpisma.ates_boyut;sayac++){
            if(yıklan_duvarlar.GetTile(new Vector3Int(pPos.x, pPos.y-1-sayac, pPos.z))==null){    
                uzaklıkalt(Carpisma.ates_boyut);
                StartCoroutine(yok(ates));
            } 
            if(yıkılmayan.GetTile(new Vector3Int(pPos2.x, pPos2.y-1-sayac, pPos2.z))!=null){  
                uzaklıkalt2(Carpisma.ates_boyut);
                if(a1==1){
        ...
        for(sayac=0;sayac<Carpisma.ates_boyut;sayac++){
            if(yıklan_duvarlar.GetTile(new Vector3Int(pPos.x, pPos.y+1+sayac, pPos.z))==null){
                uzaklıküst(Carpisma.ates_boyut);
                StartCoroutine(yok(ates));
            } 
            if(yıkılmayan.GetTile(new Vector3Int(pPos2.x, pPos2.y+1+sayac, pPos2.z))!=null){
                uzaklıküst2(Carpisma.ates_boyut);
                if(b1==1){
                ...
                if(b==1){
                    for(int i=0;i<Carpisma.ates_boyut-kısaüst;i++){
        ...
        for(sayac=0;sayac<Carpisma.ates_boyut;sayac++){
            ...
            if(yıkılmayan.GetTile(new Vector3Int(pPos2.x-1-sayac, pPos2.y, pPos2.z))!=null){ 
                ...
                if(c==1){
                    for(int i=0;i<Carpisma.ates_boyut+1-kısasol;i++){
                        ates.transform.GetChild(2).transform.localScale+=new Vector3((-1),0,0);
                        ates.transform.GetChild(2).transform.position+=new Vector3(0.5f,0,0);
                    }
                }
        ...
        for(sayac=0;sayac<Carpisma.ates_boyut;sayac++){
            if(yıklan_duvarlar.GetTile(new Vector3Int(pPos.x+1+sayac, pPos.y, pPos.z))==null){
                uzaklıksag(Carpisma.ates_boyut);
                StartCoroutine(yok(ates));
            } 
            if(yıkılmayan.GetTile(new Vector3Int(pPos2.x+1+sayac, pPos2.y, pPos2.z))!=null){ 
                uzaklıksag2(Carpisma.ates_boyut);
                if(d1==1){
                ...
                if(d==1){
                    for(int i=0;i<Carpisma.ates_boyut+1-kısasag;i++){
                        ates.transform.GetChild(1).transform.localScale+=new Vector3((-1),0,0);
                        ates.transform.GetChild(1).transform.position+=new Vector3(-0.5f,0,0);
                    }
                }
        ...
        void uzaklıküst2(int x){
            ...
            while(i<x){
                if(yıkılmayan.GetTile(new Vector3Int(pPos2.x, xx-1+i, pPos2.z))!=null){
                    kısaüst2 =(Carpisma.ates_boyut)-(i+1); Debug.Log("kısaüst2: "+kısaüst2); //atesboyut-uzklk
                    i=x;
                }
                else{i++;}
            }
        ...
        void uzaklıkalt2(int x){
            ...
            while(i<x){
                if(yıkılmayan.GetTile(new Vector3Int(pPos2.x, xx-1-i, pPos2.z))!=null){
                    kısaalt2 =(Carpisma.ates_boyut)-(i+1);
                    i=x;
                }
                else{i++;}
            }
        ...
        void uzaklıksol2(int x){
            ...
            while(i<x){
                if(yıkılmayan.GetTile(new Vector3Int(xx-i, pPos2.y, pPos2.z))!=null){
                    kısasol2 = (Carpisma.ates_boyut)-(i+1);
                    i=x;
                }
                else{i++;}
            }
        ...
        void uzaklıksag2(int x){
            ...
            while(i<x){
                if(yıkılmayan.GetTile(new Vector3Int(xx+i, pPos2.y, pPos2.z))!=null){
                    kısasag2 =(Carpisma.ates_boyut)-(i+1);
                    i=x;
                }
                else{i++;}
            }
